...
cat(
  "Found",
  length(fcs_files),
  "spectralunmixR unmixed FCS files\n"
)
...
process_fcs <- function(fcs_file) {
  ...
  # Select SSC channel according to cytometer
  if (cytometer == "Aurora") {
  ...
  # Refresh metadata after rebuilding the flowFrame
  meta <- fcs_data@parameters@data
  cat("\nAvailable channels:\n")
  print(meta$name)
  meta$desc[is.na(meta$desc)] <- ""
  ...
  compensaid_result <- tryCatch({
    ...
    # Keep all fluorescence channels
    ...
    # Heatmap
    # Keep NA values so they appear as a separate colour
